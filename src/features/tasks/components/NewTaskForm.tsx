...
import { useCreateTaskMutation, useTagsQuery, useCreateTagMutation } from '../api/tasks.api';

// ─── Priority Config ──────────────────────────────────────────────────────────
const PRIORITIES = [
  { value: 'low', label: 'Baja', className: 'hover:bg-surface-container-high focus:ring-outline', selectedClass: 'bg-surface-container-high border-outline' },
...
export const NewTaskForm = () => {
  ...
  const addTag = async () => {
    ...
    // Do not add if already in watchTags (assuming we find by matching ID or name)
    ...
    // Otherwise, create the tag via API
    try {
  ...
  const selectExistingTag = (id: string) => {
    if (!watchTags.includes(id)) {
      setValue('tags', [...watchTags, id]);
    }
  };
  ...
  const onSubmit = (data: NewTaskFormValues) => {
    // Only send the fields expected by CreateTaskDto
    createTaskMutation.mutate({
      title: data.title,
      description: data.description,
      priority: data.priority,
      tagIds: data.tags, // send the UUIDs
  ...
  return (
    <div className="bg-surface font-body text-on-surface min-h-screen">
      {/* TopNavBar */}
      <header className="fixed top-0 w-full z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl shadow-sm dark:shadow-none border-b border-slate-100 dark:border-slate-900">
        <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/dashboard')}>
            <h1 className="text-2xl font-semibold tracking-tight font-serif text-indigo-950 dark:text-indigo-50 hover:opacity-80 transition-opacity">
              Notas Arranca
            </h1>
          </div>
        ...
        <div className="w-full max-w-3xl">
          ...
          <div className="mb-12 text-center md:text-left">
            ...
            <h3 className="font-headline text-4xl lg:text-5xl font-black text-primary leading-tight">
              Definir el propósito de su próxima <span className="italic font-normal">obra maestra</span>.
            </h3>
          </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
              ...
              {createTaskMutation.isError && (
                <div className="p-4 bg-error/10 text-error rounded-xl border border-error/20 font-label text-sm">
                  Ocurrió un error al guardar la tarea. Por favor, intente nuevamente.
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Escriba aquí los detalles scholarly de esta asignación..."
                  rows={4}
                />
              </div>

              {/* Date Section (UI Only) */}
                        ...
                        return (
                          <button
                            ...
                            className={`px-6 py-2 rounded-full border-2 font-headline text-sm font-bold transition-all ${
                              isSelected
                                ? p.selectedClass
                                : `border-outline-variant/30 text-on-surface-variant ${p.className}`
              ...
              <div className="space-y-4">
                ...
                {/* Available existing tags to pick */}
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  {watchTags.map((tagId) => {
                    ...
                    const displayName = t ? t.name : '...';
                    return (
                      <span
                        key={tagId}
                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-surface-container-highest text-on-surface-variant font-label text-xs font-medium group"
                      >
                        #{displayName}
                        <button
                          type="button"
                        ...
                        return (
                          <button
                            ...
                            className={`w-10 h-10 rounded-full ${c.bgClass} border-4 transition-transform ${isPending ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110 active:scale-95'} ${
                              isSelected
                                ? `border-surface ring-1 ${c.ringClass}`
                                : 'border-transparent hover:border-surface ring-0 hover:ring-1 hover:ring-on-surface-variant'
                            }`}
                          />
                        );
                      })}
                ...
                <button
                  ...
                  className={`w-full py-5 rounded-xl font-headline text-lg font-bold shadow-xl flex items-center justify-center gap-3 transition-all ${
                    isPending
                      ...
                      : 'bg-gradient-to-r from-primary to-primary-container text-[#ffffff] shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 active:scale-[0.98]'
                  }`}
                >
                  {isPending ? (
                    <>
